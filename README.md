...
После публикации на GitHub Pages ссылка будет выглядеть примерно так:
...
Проект специально оставлен без сборщика: GitHub Pages может раздавать его как обычную статическую страницу.
...
| `.gitignore` | Исключает локальные PDF/PNG-файлы из публичного репозитория |
...
7. Откройте опубликованную ссылку GitHub Pages.

## Безопасность

`ApiTokenInstance` не хранится в коде и не должен попадать в репозиторий.

Токен вводится вручную в браузере во время тестирования. Не коммитьте реальные токены, номера телефонов и приватные данные.

Если токен был случайно опубликован или передан третьим лицам, обновите его в личном кабинете GREEN-API.
